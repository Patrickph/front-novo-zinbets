...
export const MinesContext = createContext<MinesContextType>(
  {} as MinesContextType
);

export const MinesContextProvider = ({
  children,
}: MinesContextProviderProps) => {
  ...
  const [start] = useSound(
    `/${process.env.NEXT_PUBLIC_SITE_NAME}/sound/start.mp3`,
    { volume }
  );
  const [bomb] = useSound(
    `/${process.env.NEXT_PUBLIC_SITE_NAME}/sound/bomb.mp3`,
    { volume }
  );
  const [gem] = useSound(
    `/${process.env.NEXT_PUBLIC_SITE_NAME}/sound/gem.mp3`,
    { volume }
  );
  ...
  const startGame = async ({
    ...
    try {
      const response = await api
        .post<MinesResponse>("/games/mines", {
          value_to_bet,
          number_of_bombs,
        })
        .then((response) => {
  ...
  const fetchGame = async () => {
    await api.get<MinesResponse>("/games/mines").then((response) => {
      setGame(response.data.game);
    });
  };
  ...
  const setClick = async (position: number) => {
    ...
    if (!game || game?.finish || !isLogged) {
      return;
    }

    await api
      .post<MinesResponse>("/games/mines/play", {
        position,
      })
      .then((response) => {
        setGame(response.data.game);
  ...
  const setCashout = async () => {
    setIsLoading(true);
    await api
      .post<MinesResponse>("/games/mines/cashout")
      .then((response) => {
        setGame(response.data.game);
        gem();
      })
