...
const Input = (
  ...
  const setCurrencyValue = (event: React.FormEvent<HTMLInputElement>) => {
    const onlyNumbers = event.currentTarget.value.match(/\d/g)?.join("") ?? ""

    let finalValue = moneyMask(onlyNumbers) as number

    let value = (currency ? formatBRL(finalValue) : finalValue) as string

    event.currentTarget.value = value
  }

  return (
    <div className="floating   mb-3 relative rounded">
      {mask ? (
        <InputMask
          mask={mask}
          maskChar={maskChar}
          ...
          className={`${
            errors && "border-red-500 border rounded-b-none"
      ...
      <label
        className="flex items-center gap-2 mb-2 uppercase  text-zinc-400 text-xs font-bold absolute top-0 left-0 px-4 py-[1.35rem] h-full pointer-events-none transform origin-left transition-all duration-100 ease-in-out "
        htmlFor={id}
      >
        {icon}
        {title}
