...
const schema = yup
  .object({
    pix_key_type: yup
      ...
      .oneOf(
        ["cpf", "email", "phone", "random", "cnpj"],
        "Tipo de chave inválida"
      ),
    pix_key: yup.string().required("Chave obrigatória"),
  })
  .required()
...
export default function Pix() {
  ...
  const {
    ...
    defaultValues: async () =>
      api.get("/account/profile").then(response => response.data.user),
  })
  ...
  const onSubmit = async ({ pix_key_type, pix_key }: IFormInputs) => {
  ...
  return (
    <div className="rounded border border-white/10 bg-[#0d0716] p-6">
        ...
        <div className="space-y-2">
            ...
            <div className="sm:col-span-2">
              <div className="floating mb-3 relative">
                <select
                  ...
                  } disabled:cursor-not-allowed bg-[#07050a] text-base text-zinc-300 appearance-none block w-full px-4 h-14 leading-10 rounded shadow-sm placeholder-gray-400 focus:outline-none focus:ring-0`}
                >
            ...
            <div className="sm:col-span-4">
              <Input
                ...
                mask={
                  watchPixKeyType === "cpf"
                    ? "999.999.999-99"
                    : watchPixKeyType === "cnpj"
                    ? "99.999.999/9999-99"
                    : watchPixKeyType === "phone"
                    ? "(99) 99999-9999"
                    : null
                }
          ...
          <div className="flex items-center justify-between gap-x-6">
            <span className="text-xs text-zinc-400">
              *Sua chave Pix deve estar ligada ao seu CPF
            </span>

            <button
              type="submit"
              className=" flex items-center gap-1 cursor-pointer md:mx-2 rounded shadow-green bg-[#8845fa] hover:bg-[#713ad6] text-white  px-2 py-2 text-center font-bold text-xs md:text-sm"
            >
