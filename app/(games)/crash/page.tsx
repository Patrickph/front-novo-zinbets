...
export default function CrashPage() {
  ...
  const onSubmit = async ({
    ...
    setAutoCashout(
      value_auto_cashout
        ? Number(value_auto_cashout?.replace(/[^0-9]/g, ""))
        : 0
    );
  ...
  return (
    <>
      <div className="rounded bg-card p-6 flex flex-col gap-6">
        <div className="grid grid-cols-3 gap-4">
          <form
            onSubmit={handleSubmit(onSubmit)}
            className="border-t pt-4 lg:pt-0 lg:border-t-0 lg:border-r lg:pr-3 border-border-primary  order-last lg:order-first col-span-3 lg:col-span-1"
          >
            <Input
            ...
            <div className="flex -mt-2 mb-4 items-center justify-between rounded overflow-hidden">
              <button
                ...
                className="disabled:cursor-not-allowed disabled:hover:bg-zinc-800 disabled:hover:text-zinc-400 h-10 text-sm flex justify-center items-center cursor-pointer font-semibold bg-zinc-800 hover:text-zinc-950 hover:bg-primary flex-1 text-center text-zinc-400"
              >
                1/2
              </button>
              <button
            ...
            {!isBet ? (
              waitBet > 0 ? (
                <div
                  ...
                  className="h-12 flex items-center text-center w-full justify-center cursor-pointer  rounded shadow-green shadow-red-500 bg-red-500 text-foreground hover:bg-red-700 px-2 py-2  font-bold text-xs md:text-sm"
                >
                  Na fila (clique para cancelar)
                </div>
              ) : (
                <Button className="w-full rounded" size={"lg"} type="submit">
                  Começar o jogo
                </Button>
              )
            ) : (
              <button
                ...
                type="button"
                className="disabled:cursor-not-allowed disabled:opacity-80 h-12 w-full text-center cursor-pointer  rounded shadow-green shadow-red-500 bg-red-500 text-foreground hover:bg-red-700 px-2 py-2  font-bold text-xs md:text-sm"
              >
                {game?.status === "pending"
                  ? "Aguardando"
                  : `Retirar ${formatBRL(
                      ((bet.bet ?? 0) * (game?.multiplier ?? 1)) / 100
                    )}`}
              </button>
            )}
          </form>
          ...
          <div className="col-span-3 lg:col-span-2 flex flex-col gap-4 ">
            ...
            <div className="w-full relative aspect-video bg-zinc-900 rounded overflow-hidden p-3">
              <ChartCrash
                started_at={game?.started_at}
                float={game?.multiplier ?? 0}
              />

              {!!game && !!game.pending_at && game?.status == "pending" && (
                <LoadingNextGameCrash date={game.pending_at} />
              )}
              ...
              {!!game &&
                ...
                game?.multiplier > 1 && (
                  <div className="absolute top-0 left-0 w-full h-full  flex flex-col items-center justify-center">
                    {bet && bet.win && (
                      <div className="mb-4 w-fit h-fit text-center font-bold bg-primary shadow overflow-hidden rounded flex items-center flex-col justify-center">
                        <div className="flex-1 flex items-center text-xl px-3 py-3 drop-shadow text-contrast">
                          X{bet?.multiplier ?? 1}
                        </div>
                        <div className="text-contrast font-bold w-full h-10 flex justify-center items-center text-xs bg-green-700/40 p-2">
                          VOCÊ GANHOU{" "}
                          {formatCentsToBRL(
                            (bet?.bet ?? 1) * (bet?.multiplier ?? 1)
                          )}
                        </div>
                      </div>
                    )}

                    <div className="w-fit px-6 h-20 text-center font-bold backdrop-blur-sm bg-zinc-700/30 shadow overflow-hidden rounded flex items-center flex-col justify-center">
              ...
              {!!game && game?.status === "crashed" && (
                <div className="absolute top-0 left-0 w-full h-full  flex items-center justify-center">
                  <div className="w-fit h-fit text-center font-bold  bg-red-500/80 shadow overflow-hidden rounded flex items-center flex-col justify-center">
            ...
            <div className="hidden lg:block w-full border-t border-border-primary pt-4">
              <LastGamesCrash />
            </div>
          </div>
        </div>
